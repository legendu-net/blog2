...
with Path("words.yml").open(encoding="utf-8") as _:
...
def _parse_record(path: Path):
    return Post(path).record()


def parse_records() -> list[Record]:
    logger.info("Parsing posts into records ...")
    paths = list(
        path for path in itertools.chain(
            BASE_DIR.glob("docs/articles/????/??/*/index.*"),
            BASE_DIR.glob("docs/drafts/????/??/*/index.*"),
            BASE_DIR.glob("docs/outdated/????/??/*/index.*"),
        ) if is_post(path)
    )
...
class Post:
    ...
    def blog_dir(self):
        """Get the corresponding blog directory (articles, drafts, or outdated) of a post."""
        return self.path.parent.parent.parent.parent.stem

    def update_after_move(self) -> None:
        """Update the disclaimer after the post is moved to a different directory.
        """
        if self.is_markdown():
    ...
    def _update_after_move_notebook(self) -> None:
        ...
        if blog_dir in (DRAFTS, OUTDATED):
            DISCLAIMER = DISCLAIMER_DRAFTS if blog_dir == DRAFTS else DISCLAIMER_OUTDATED
            DISCLAIMER_OTHER = (
                DISCLAIMER_OUTDATED if blog_dir == DRAFTS else DISCLAIMER_DRAFTS
            )
            if notebook["cells"][1]["source"][0].strip() == DISCLAIMER_OTHER.strip():
                notebook["cells"][1]["source"][0] = DISCLAIMER
            if notebook["cells"][1]["source"][0].strip() != DISCLAIMER.strip():
                notebook["cells"].insert(
                    1, {"source": [DISCLAIMER], "cell_type": "markdown", "metadata": {}}
                )
            self._write_notebook(notebook)
        elif notebook["cells"][1]["source"][0].strip() in (
            DISCLAIMER_DRAFTS.strip(),
            DISCLAIMER_OUTDATED.strip(),
        ):
    ...
    def _update_after_move_markdown(self) -> None:
        ...
        if blog_dir in (DRAFTS, OUTDATED):
            ...
            with self.path.open("w", encoding="utf-8") as fout:
                fout.writelines(lines[:index+1])
                fout.writelines(DISCLAIMER)
                index_10 = index + 10
                fout.writelines(
                    line for line in lines[index+1:index_10]
                    if line.strip() not in (DISCLAIMER_DRAFTS.strip(), DISCLAIMER_OUTDATED.strip())
                )
                fout.writelines(lines[index_10:])
            return
        text = (
            self.path.read_text(encoding="utf-8")
            .replace(DISCLAIMER_DRAFTS, "")
    ...
    def _parse_markdown(self) -> Record:
        ...
        return Record(
            str(self.path.relative_to(BASE_DIR)),
            self.blog_dir(),
            status,
            date,
            modified,
    ...
    def _create_notebook(self, title: str):
        ...
        if self.blog_dir() == DRAFTS:
            text = text.replace("${DISCLAIMER}", DISCLAIMER_DRAFTS)
        else:
    ...
    def _create_markdown(self, title: str):
        with self.path.open("w", encoding="utf-8") as fout:
            ...
            if self.blog_dir() == DRAFTS:
                fout.writelines("\n")
                fout.writelines(DISCLAIMER_DRAFTS)
                fout.writelines("\n")
...
class Blogger:
    ...
    def _move_1(self, src: AnyPath, dst: AnyPath) -> None:
        ...
        if isinstance(src, str):
            src = Path(src)
        if dst in (ARTICLES, DRAFTS, OUTDATED):
