...
date: 2026-04-15 19:27:01.510045
authors:
  - bendu
...
1. A good design is essential in template meta programming.
...
1. If a type depends on a template argument,
...
1. Whenever you refer to a template class,
...
1. A specialization of a template class does not have access to members in the
...
1. Template functions/methods can only be "full" specialized.
...
1. The implementations of specializations of a template method must be outside
...
1. Except types, you can also use values that are essentially integers
...
1. Specializing a template class is often tedious.
   ...
   1. If different versions of a template class requires different temporary variables,
...
1. Functions operate on containers in STL often take iterators as arguments,
...
1. If you want to member types of a template type, add `typename` before it.
