...
date: 2026-04-13 23:33:13.597159
authors:
  - bendu
label: model-fitting-in-anova-analysis
license: CC-BY-4.0
tags:
  - model
  - R
...
1. In anova, more often we data in hand and want to create the design
...
1. 函数lme()使用$\mid$来分隔低层因此和Group因子， 用/表示因子的嵌套结构。

1. 函数lmer()使用小括号来表示Random Effect， 亦即小括号内的表达式部分表示随机效应， 不在小括号里面的部分表示固定效应。
...
1. Function `median` returns a vector of length one of two depending on
...
1. Function `summary` can give some statistics about the data. But not
   as specific as SAS does.

1. `rstudent` extracts studentized residuals of models from objects
...
1. `IQR` calculate the interquartile range of a given data.
