...
1. Too much usage of **off-heap** memory.
...
1. data skew (e.g., big data table but not partitioned)

1. Some tables in joins are too large.
...
1. Reducing the number of executor cores (which helps reducing memory consumption).
   For example,
...
1. Configure the JVM option `MaxDirectMemorySize`
