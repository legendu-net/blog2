...
date: 2026-04-15 19:27:01.453374
authors:
  - bendu
label: hive-sql
license: CC-BY-4.0
...
1. use `%` rather than `*` for matching multiple characters

1. use quit or exit

1. when filtering use the where clause
   while when joining use the on clause

1. it seems that it's not necessary to use group before running aggregation functions
   in hive, e.g., when you count the number of records in a table.

1. must separate fields/column names with comma in the select clause or group by clause.

1. it seems to me that Hive runs command from right to left?

1. Random sample from a Hive table.
