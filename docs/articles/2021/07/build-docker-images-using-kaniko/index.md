...
date: 2026-04-13 23:14:10.192291
authors:
  - bendu
...
1. Even if you use an official Kaniko Docker image,
...
1. It might be helpful to keep a Kaniko pod running
...
1. The credential file `config.json` for authentication
...
1. It is suggested that you always pass the option `--log-timestamp`
...
1. The option `--cleanup` (of `/kaniko/executor`) cleans up the filesystem/layers
...
1. Always, specify `--push-try` (e.g., `--push-retry=2`)
