...
date: 2026-04-15 19:27:00.261055
authors:
  - bendu
label: tips-on-pex
license: CC-BY-4.0
...
1. Python packages that have native code (C/C++, Fortran, etc.) dependencies work well in pex.
   However,
...
1. If there are multiple versions of Python installed in your system,
...
1. It is suggested that you turn on verbosity mode
   using the option `-v`
...
1. A conda-pack environment is a `tar.gz` file and need to be decompressed before being used
   while a pex file can be used directly.

1. If a Python interpreter exists,
